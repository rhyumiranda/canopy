...
# --- durable lifecycle event: a merged PR writes one one-shot event, consumed once ---
R3="$(new_repo)"; cd "$R3"; "$CANOPY" init >/dev/null 2>&1
ID3="$("$CANOPY" task add "shipped for events" 2>/dev/null)"
"$CANOPY" task set "$ID3" pr 3 >/dev/null
"$CANOPY" task set "$ID3" worktree "$R3" >/dev/null
"$CANOPY" task set "$ID3" agent claude >/dev/null
"$CANOPY" task checkpoint "$ID3" "ready_for_review" >/dev/null
"$CANOPY" task status "$ID3" pr-open >/dev/null
CANOPY_STOP_MARKER="$WORK/w3-stopped" PATH="$STUB:$PATH" "$CANOPY" watch once >/dev/null 2>&1 || true
[ "$(jq -r '.tasks[0].status' "$R3/.canopy/state.json")" = "done" ] && ok "watch once marks merged task done" || bad "watch once did not mark done"
[ "$(jq -r '.events|length' "$R3/.canopy/events/lifecycle.json")" = "1" ] && ok "watch once writes one durable event" || bad "durable event missing"
jq -e '.events[0] | select(.task_id=="'"$ID3"'" and .status=="done" and .checkpoint=="ready_for_review")' "$R3/.canopy/events/lifecycle.json" >/dev/null \
  && ok "event records task/status/checkpoint" || bad "event payload wrong"
# rerun does not duplicate the same transition
...
START="$(date +%s)"
PATH="$STUB:$PATH" "$CANOPY" events wait 0 >/dev/null 2>&1 && bad "wait 0 should fail when no event" || ok "wait 0 returns when no event"
END="$(date +%s)"
