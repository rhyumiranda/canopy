...
# A task carrying legacy Herdr fields still recovers via the generic re-spawn
# guidance on stable — interactive Herdr resume/reconcile are experimental-only.
IDH="$("$CANOPY" task add "legacy herdr fields" 2>/dev/null)"
"$CANOPY" task set "$IDH" brief "resume this" >/dev/null
...
echo "$BRIEFH" | grep -q "Re-spawn a worker" && ok "recover uses generic re-spawn even with legacy Herdr fields" || bad "recover missing generic re-spawn"
echo "$BRIEFH" | grep -qiE "worker resume|worker reconcile" && bad "recover must not emit Herdr resume/reconcile on stable" || ok "recover emits no Herdr resume/reconcile on stable"
