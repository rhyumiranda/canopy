...
case "$1 ${2:-}" in
  ...
  tab\ create) printf '%s\n' '{"id":"tab-1"}' ;;
  agent\ start) printf '%s\n' '{"pane_id":"pane-1","agent_session_id":"herdr-session-1"}' ;;
...
[ "$(jq -r .herdr_pane_id "$TF")" = pane-1 ] && ok 'pane id persisted' || bad 'pane id missing'
...
[ "$(grep -c 'tab create' "$WORK/herdr.log")" = 1 ] && ok 'duplicate start reuses tab' || bad 'duplicate tab created'
