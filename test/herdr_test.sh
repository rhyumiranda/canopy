...
# the launch wires a Stop hook via --settings pointing at a per-worker settings file
grep -q -- "agent start .* -- claude --dangerously-skip-permissions --settings .*worker-settings/$IDIDLE.json" "$WORK/herdr.log" \
  && ok 'Claude launch seeds a Stop hook via --settings' || bad 'Claude launch missing --settings Stop hook'
