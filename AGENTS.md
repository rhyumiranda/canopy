...
- Workers inherit CANOPY_ROLE=orchestrator, so the guard-project-write Bash hook rejects any command containing a bare redirect character, an arrow (dash then angle), or an angle-bracket email trailer; commit and checkpoint by writing the text to a scratch file under /tmp and passing it to `git commit -F` / canopy via that path (the allowlist matches the /tmp path), not by typing those characters inline.
