...
## The loop, per task
1. Capture intent → `canopy task add "<title>"` → get `<id>`. Set the **brief** (what to build, for the worker) and the **why** (the rationale, for the PR): `canopy task set <id> brief "…"` and `canopy task set <id> why "…"`. (The PR renders Summary / What / Why as separate sections — What comes from the brief, Why from the why.)
